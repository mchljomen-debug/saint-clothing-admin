...
const[selectedBranch,setSelectedBranch]=useState(role==="admin"?"all":branch);
...
const formatTimeAMPM=(date)=>date?date.toLocaleTimeString([],{
hour:"numeric",
minute:"2-digit",
hour12:true
}):"--";
...
axios.get(`${backendUrl}/api/branch/list`,{
headers:{Authorization:`Bearer ${token}`}
}).catch(()=>({data:{branches:[]}}))
]);
...
?Math.floor((netProfit/totalRevenue)*100)
:0;
...
const rangedOrders=filterOrdersByRange(orders,categoryRange).filter(isPaidOrder);
...
const rangedOrders=filterOrdersByRange(
orders,
topProductsRange
).filter(isPaidOrder);
...
className="min-w-[92px] rounded-[5px] px-3 py-2.5 text-xs outline-none bg-white text-gray-900 border border-black/10 shadow-sm"
>
...
<h3 className={`text-sm sm:text-[17px] font-black uppercase tracking-[0.08em] ${textStrong}`}>
{title}
</h3>
...
<span className={`text-xs sm:text-sm font-semibold ${textStrong} truncate`}>
{label}
</span>
</div>

<span className={`text-xs sm:text-sm font-black ${textStrong} ml-3`}>
{data[index]||0}
</span>
</div>
))}
...
<div
key={i}
className="h-32 rounded-[5px] bg-white/70"
/>
))}
</div>
...
<aside className={`${panelBg} rounded-[5px] p-0 xl:sticky xl:top-24 xl:h-fit overflow-hidden`}>
...
<span className={dailyTrend.isUp?"text-green-600":"text-red-500"}>
{dailyTrend.percent}%
</span>

<span className="text-[#0A0D17]/45">
vs previous point
</span>
</div>
</div>
...
<div className={`${softPanelBg} rounded-[5px] p-4 min-w-0 overflow-hidden transition hover:shadow-md`}>
...
<span className={profitTrend.isUp?"text-green-600 font-semibold":"text-red-500 font-semibold"}>
{profitTrend.percent}%
</span>
...
<div className={`${panelBg} xl:col-span-2 rounded-[5px] p-4 sm:p-5`}>
{renderSectionHeader(
"Sales Trend",
...
<div className={`${lowStockProducts.length>5?"max-h-[320px] overflow-y-auto pr-1":""} space-y-2`}>
...
<th className="py-3 bg-[#f7f7f4] font-bold">
Date
</th>
</tr>
</thead>
